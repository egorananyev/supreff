...
shams$fasP <- shams$fas / shams$n
p <- ggplot(shams, aes(x=SubjID, y=fasP)) + geom_bar(stat='identity') + 
...
library(BayesFactor)
mdB <- md[!is.na(md$RT),]
mdB$trial <- as.factor(mdB$trial)
mdB$SubjID <- as.factor(mdB$SubjID)
mdB$mpRotMask_ctr <- as.factor(mdB$mpRotMask_ctr)
mdB$maskSpeed_ctr <- as.factor(mdB$maskSpeed_ctr)
mdB$mpRotTarg_ctr <- as.factor(mdB$mpRotTarg_ctr)
bfFull <- anovaBF(RT ~ SubjID + trial + mpRotMask_ctr * mpRotTarg_ctr * maskSpeed_ctr, 
                  data=mdB, whichRandom = c('SubjID'))
