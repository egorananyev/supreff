...
p <- ggplot(mdn, aes(x=maskSpeed, y=subjNormRT, colour=motPat, group=motPat)) +  
    stat_smooth(method='loess', alpha=.15, aes(fill=motPat), show.legend=F, size=.6, span=.8) + 
    ...
    theme(panel.grid.minor.x=element_blank(), panel.grid.minor.y=element_blank(),
          ...
          legend.background = element_rect(fill='transparent'))
plot(p)
if(out){png('expt1loess.png', width=3.5, height=2, units='in', res=600); plot(p); dev.off()}
```
...
plot(p)
if(out){png('expt1lt.png', width=2.5, height=2, units='in', res=600); plot(p); dev.off()}
p <- ggplot(mdn[mdn$subjThresh>median(mdn$subjThresh),],
            aes(x=maskSpeed, y=subjNormRT, colour=motPat, group=motPat)) +  
    stat_smooth(method='loess', alpha=.15, aes(fill=motPat), show.legend=F, size=.6) + 
    geom_point(data=ds[ds$subjThresh>median(mdn$subjThresh),], 
...
plot(p)
if(out){png('expt1ht.png', width=3.5, height=2, units='in', res=600); plot(p); dev.off()}
```
...
p <- ggplot(mdnsum, aes(x=maskSpeed, y=meanCondST, colour=motPat, group=motPat,
    ...
    labs(x=expression(paste('Mask Speed (', degree, '/s)', sep='')), y='Normalized Suppression Time',
         colour='   Motion Pattern') + ylim(.7,1.4) +
    guides(colour=guide_legend(keyheight=.3, default.unit='inch')) +
    theme(panel.grid.minor.x=element_blank(), panel.grid.minor.y=element_blank(),
          axis.text=element_text(size=8), axis.title=element_text(size=9),
          legend.text=element_text(size=8), legend.title=element_text(size=9),
          legend.key = element_blank(), legend.margin=unit(-.04, 'in'),
          legend.background = element_rect(fill='transparent'))
if(!out){plot(p)}else{png('expt1lin4.png', width=3.5, height=2.1, units='in', res=600); plot(p); dev.off()}
```

### Expected pattern: FSS for pattern and speed
```{r, fig.height=4, fig.width=7}
expData = mdnsum
...
p <- ggplot(expData, aes(x=maskSpeed, y=meanCondST, colour=motPat, group=motPat)) +
    ...
    theme(panel.grid.minor.x=element_blank(), panel.grid.minor.y=element_blank(),
          ...
          legend.key = element_blank(), legend.margin=unit(-.04, 'in'),
          legend.background = element_rect(fill='transparent'))
if(!out){plot(p)}else{png('expt1linPred1.png', width=3.5, height=2.1, units='in', res=600); plot(p); dev.off()}
```

### Expected pattern: FSS only for pattern
```{r, fig.height=4, fig.width=7}
expData = mdnsum
expData$seCondST <- 0.08
expData[expData['motPat']=='Linear Target\nLinear Mask', 'meanCondST'] <- 
...
p <- ggplot(expData, aes(x=maskSpeed, y=meanCondST, colour=motPat, group=motPat)) +
    ...
    theme(panel.grid.minor.x=element_blank(), panel.grid.minor.y=element_blank(),
          ...
          legend.key = element_blank(), legend.margin=unit(-.04, 'in'),
          legend.background = element_rect(fill='transparent'))
if(!out){plot(p)}else{png('expt1linPred2.png', width=3.5, height=2.1, units='in', res=600); plot(p); dev.off()}
```

### Expected pattern: Salience-driven
```{r, fig.height=4, fig.width=7}
expData = mdnsum
expData$seCondST <- 0.08
expData[expData['motPat']=='Linear Target\nLinear Mask', 'meanCondST'] <- 
...
p <- ggplot(expData, aes(x=maskSpeed, y=meanCondST, colour=motPat, group=motPat)) +
    ...
    theme(panel.grid.minor.x=element_blank(), panel.grid.minor.y=element_blank(),
          ...
          legend.key = element_blank(), legend.margin=unit(-.04, 'in'),
          legend.background = element_rect(fill='transparent'))
if(!out){plot(p)}else{png('expt1linPred3.png', width=3.5, height=2.1, units='in', res=600); plot(p); dev.off()}
```
