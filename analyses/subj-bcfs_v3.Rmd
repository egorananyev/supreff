...
ggplot(subjList, aes(x=altSubj, y=subjThresh)) + geom_bar(stat='identity') + 
    theme_black() + labs(y='Threshold Value') + 
...
ggplot(md, aes(x=maskSpeed, y=RT, colour=factor(motPat), group=factor(motPat))) +  
    stat_smooth(method='lm', formula=y~ns(x,3), alpha=.1, aes(fill=factor(motPat)), show.legend=F) +
...
if(out){pdf("expt3bw.pdf", width=5, height=3)}
ggplot(mdn, aes(x=maskSpeed, y=subjNormRT, colour=motPat, group=motPat)) +  
    stat_smooth(method='lm', formula=y~ns(x,3), alpha=.1, aes(fill=motPat), show.legend=F) + 
...
ggplot(mdn[mdn$subjThresh>median(mdn$subjThresh),],
    ...
    stat_smooth(method='lm', formula=y~ns(x,3), alpha=.15, aes(fill=motPat), show.legend=F) + 
