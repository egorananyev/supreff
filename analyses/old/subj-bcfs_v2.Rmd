...
```{r}
m_mpMatch_full_ctr <- lmer(RT~trialN+mpMatch_ctr*maskSpeed_ctr*subjThresh_ctr+(1|SubjID),md)
pvalfn(m_mpMatch_full_ctr)
r2.corr.mer(m_mpMatch_full_ctr)
# write.table(pvalfn(m_mpMatch_full_ctr),'clipboard',sep='\t',row.names=T)
```
...
## Target/mask types
```{r}
pxt <- function(x){print(xtable(x), type='html')}
m_mpType_full_ctr <- lmer(RT~trialN+subjThresh_ctr*mpRotMask_ctr*mpRotTarg_ctr*maskSpeed_ctr+
                              (1|SubjID), md)
(pvalfn(m_mpType_full_ctr))
r2.corr.mer(m_mpType_full_ctr)
# write.table(pvalfn(m_mpType_full_ctr),'clipboard',sep='\t',row.names=T)
```
