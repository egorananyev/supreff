...
if(out){pdf("expt4bw-4.pdf", width=5, height=3)}
ggplot(mdn, aes(x=maskSpeed, y=subjNormRT, colour=factor(targSpeed))) +
    stat_smooth(method='lm', formula=y~ns(x,4), alpha=.1, aes(fill=factor(targSpeed)), show.legend=F, se=T, na.rm=T) + 
...
if(out){dev.off()}
if(out){pdf("expt4bw-loess.pdf", width=5, height=3)}
ggplot(mdn, aes(x=maskSpeed, y=subjNormRT, colour=factor(targSpeed))) +
    stat_smooth(method='loess', alpha=.1, aes(fill=factor(targSpeed)), show.legend=F, se=T, na.rm=T) + 
    geom_point(data=subjMeans, aes(x=maskSpeed, y=subjMean, colour=factor(targSpeed)), 
               position=position_dodge(width=.25), size=.7, alpha=.5) + # square is shape=7
    labs(x=expression(paste('Mask Speed (', degree, '/s)', sep='')), 
         y='Normalized RT', colour='Target Speed\n') + 
