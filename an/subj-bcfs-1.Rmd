...
p <- ggplot(subjList, aes(x=altSubj, y=subjThresh)) + geom_bar(stat='identity') + 
    ...
    theme(legend.position='none', axis.text.x = element_blank(), panel.border = element_blank(),
          axis.text=element_text(size=8), axis.title=element_text(size=9))
plot(p)
if(out){tiff('dm1thresh.tiff', width=3.5, height=2.6, units='in', res=300)}
if(out){plot(p)}
if(out){dev.off()}
if(out){pdf("dm1thresh.pdf", width=4, height=3)}
if(out){plot(p)}
if(out){dev.off()}
...
p <- themefy(p)
if(!out){plot(p)}else{png('dm1lin4.png', width=3.5, height=2.1, units='in', res=600); plot(p); dev.off()}
```
...
```{r}
m_mpMatch <- lmer(RT~trialN+mpMatch_ctr*maskSpeed_ctr+(1|SubjID),md)
pvalfn(m_mpMatch)
...
## Target/mask types
```{r}
m_mpType <- lmer(RT~trialN+mpRotMask_ctr*mpRotTarg_ctr*maskSpeed_ctr+(1|SubjID), md)
(pvalfn(m_mpType))
...
#bfTop <- anovaBF(RT ~ SubjID + trial + mpRotMask_ctr * mpRotTarg_ctr * maskSpeed_ctr, 
                  #data=mdB, whichRandom = c('SubjID'), whichModels='top')
#print(bfTop)
