---
title: "Effect of dynamic mask speed on suppression of a moving stimulus: Effect of limited trajectory length (rotational)"
author: "Egor Ananyev"
date: '2016-04-28'
output:
    html_document:
...
condDir <- paste(dataDir,'bcfs6/', sep='')
...
for(curSubj in 1:length(subjList$Subj)){ #going through all dirs
    for(curCond in 1:length(condList)){
        ...
        grepPattern <- paste('dm6_p-', subjList$Subj[curSubj], '_dom-', 
...
if(out){pdf("dm6thresh.pdf", width=4, height=3)}
...
p <- ggplot(mdnsum, aes(x=maskSpeed, y=meanCondST, colour=factor(targSpeed), 
    ...
    theme(panel.grid.minor.x=element_blank(), panel.grid.minor.y=element_blank(),
          ...
          legend.key = element_blank(), #legend.margin=unit(-.04, 'in'),
          legend.background = element_rect(fill='transparent'))
if(!out){plot(p)}else{png('dm6lin.png', width=3.5, height=2.1, units='in', res=600); plot(p); dev.off()}
```
...
p <- themefy(p)
if(!out){plot(p)}else{png('dm6nb.png', width=3.5, height=2.1, units='in', res=600); plot(p); dev.off()}
```
