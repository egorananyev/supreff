...
condDir <- paste(dataDir,'bcfs5/', sep='')
subjList <- read.xlsx(file=paste(dataDir, 'Subjects.xlsx', sep=''), sheetName='bcfs5', 
...
if(out){pdf("dm5thresh.pdf", width=4, height=3)}
...
p <- ggplot(mdnsum, aes(x=maskSpeed, y=meanCondST, colour=factor(targSpeed), 
    ...
    theme(panel.grid.minor.x=element_blank(), panel.grid.minor.y=element_blank(),
          ...
          legend.key = element_blank(), #legend.margin=unit(-.04, 'in'),
          legend.background = element_rect(fill='transparent'))
if(!out){plot(p)}else{png('dm5lin.png', width=3.5, height=2.1, units='in', res=600); plot(p); dev.off()}
```
...
p <- themefy(p)
if(!out){plot(p)}else{png('dm5nb.png', width=3.5, height=2.1, units='in', res=600); plot(p); dev.off()}
```
...
### Full linear model
```{r}
m_full <- lmer(RT~targSpeed_ctr*maskSpeed_ctr+(1|SubjID),md)
pvalfn(m_full)
...
regTargSpeed <- lmBF(RT ~ targSpeed_ctr + SubjID, data=mdNoNa, whichRandom = 'SubjID')
regSum <- lmBF(RT ~ targSpeed_ctr + maskSpeed_ctr + SubjID, data=mdNoNa, 
               whichRandom = 'SubjID')
regInt <- lmBF(RT ~ targSpeed_ctr * maskSpeed_ctr + SubjID, data=mdNoNa, 
               whichRandom = 'SubjID')
regSum / regTargSpeed
regSum / regMaskSpeed
regInt / regSum
...
regTargSpeed <- lmBF(nNa ~ targSpeed_ctr + SubjID, data=dsna, whichRandom = 'SubjID')
regSum <- lmBF(nNa ~ targSpeed_ctr + maskSpeed_ctr + SubjID, data=dsna, 
               whichRandom = 'SubjID')
regInt <- lmBF(nNa ~ targSpeed_ctr * maskSpeed_ctr + SubjID, data=dsna, 
               whichRandom = 'SubjID')
regSum / regTargSpeed
regSum / regMaskSpeed
regInt / regSum
