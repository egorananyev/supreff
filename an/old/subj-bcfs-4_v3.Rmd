...
if(out){tiff('expt4loess.tiff', width=3.5, height=2, units='in', res=600); plot(p); dev.off()}
if(out){dev.off()}
p <- ggplot(subjMeans, aes(x=maskSpeed, y=subjMean, colour=factor(targSpeed))) +
    stat_smooth(method='loess', alpha=.15, aes(fill=targSpeed), show.legend=F, size=.6) + 
    geom_point(data=subjMeans, aes(x=maskSpeed, y=subjMean, colour=targSpeed), 
    ...
    scale_colour_manual(breaks=c(1,5), labels=c( expression(paste(' 1 ', degree, '/s', sep='')),
            expression(paste(' 5 ', degree, '/s', sep=''))), values=c('#E69F00', '#0072B2')) +
    theme(panel.grid.minor.x=element_blank(), panel.grid.minor.y=element_blank(),
          axis.text=element_text(size=8), axis.title=element_text(size=9),
          legend.text=element_text(size=8), legend.title=element_text(size=9),
          legend.key = element_blank(), legend.margin=unit(-.04, 'in'),
