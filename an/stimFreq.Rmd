...
# plotting the image:
plot(1:2, type='n')
rasterImage(targImS, 1.2, 1.27, 1.8, 1.73)
```
...
# plot(1:2, type='n')
# rasterImage(maskImS, 1.2, 1.27, 1.8, 1.73)
```
...
targIm <- array(NA, dim=c(dims,nFrames))
maskIm <- array(NA, dim=c(dims,nFrames))
...
## Pixelwise on target
```{r}
targSpecLo <- array(NA, dim=dims)
targSpecHi <- array(NA, dim=dims)
#TEMP: a single, example, pixel:
# pixInd <- which(targImS>.5, arr.ind=T)[350,] # x=197 y=94
ptm <- proc.time()
for(x in 1:dims[1]){
    for(y in 1:dims[2]){
        targSpec <- spectrum(targIm[x,y,1:60], plot=F)
...
for(x in 1:dims[1]){
    for(y in 1:dims[2]){
        maskSpec <- spectrum(maskIm[x,y,1:60], plot=F)
