...
condDir <- paste(dataDir,'bcfs4/', sep='')
subjList <- read.xlsx(file=paste(dataDir, 'Subjects.xlsx', sep=''), sheetName='bcfs4', 
                      colIndex=c(1:7), header=T)
...
## False alarms
```{r, fig.height=3, fig.width=4}
shams$fas <- shams$n*3 - shams$RT0 #multiplying by 3 bc there were 3 sessions
shamSum <- ddply(shams, .(SubjID), summarise, fasTot=sum(fas), nTot=sum(n*3))
...
p <- ggplot(mdnsum, aes(x=maskSpeed, y=meanCondST, colour=factor(targSpeed), 
    ...
    scale_colour_manual(breaks=c(1,5), labels=c(
        ...
        values=c('#E69F00', '#0072B2')) #ylim(.8,1.2) +
p <- themefy(p)
if(!out){plot(p)}else{png('dm4lin.png', width=3.5, height=2.1, units='in', res=600); plot(p); dev.off()}
```

## Non-breaking trials
```{r, fig.height=2.1, fig.width=3.5}
# computing mean and SD of subj means:
mdnsNb <- ddply(ds, .(SubjID, subjThresh, targSpeed, maskSpeed), summarise,
                meanSubjNb = mean(cnt_RTna/24, na.rm=T))
mdnsumNb <- ddply(mdnsNb, .(targSpeed, maskSpeed), summarise,
                meanCondNb = mean(meanSubjNb),
                seCondNb = sd(meanSubjNb) / sqrt(length(SubjID)))
mdnsumNb$targSpeed <- as.factor(mdnsumNb$targSpeed)
#mdnsumNb$maskSpeed <- as.factor(mdnsumNb$maskSpeed)
# plot
p <- ggplot(mdnsumNb, aes(x=maskSpeed, y=meanCondNb, colour=targSpeed, group=targSpeed,
    ...
    scale_colour_manual(breaks=c(1,5), labels=c(
        ...
        values=c('#E69F00', '#0072B2')) #ylim(.8,1.2) +
p <- themefy(p)
if(!out){plot(p)}else{png('dm4nb.png', width=3.5, height=2.1, units='in', res=600); plot(p); dev.off()}
```
...
```{r, fig.height=2.1, fig.width=3.5}
expData <- mdnsum
expData$seCondST <- 0.03
expData$meanCondST <- c(1.05,1.1,1.05,.95,.9,.85,
                        .85,.9,.95,.97,1,.95)
#expData$meanCondST <- expData$meanCondST + .075
#expData$meanCondST <- expData$meanCondST + runif(10,.01,.03)
#expData$meanCondST <- expData$meanCondST + c(0,0,.3,.3,.1,.1,-.1,-.1,-.1,-.1,-.1,-.1)
p <- ggplot(expData, aes(x=maskSpeed, y=meanCondST, colour=factor(targSpeed), 
                        group=factor(targSpeed), 
                        ymax=meanCondST+seCondST,  ymin=meanCondST-seCondST)) +
    geom_point(position=dodge, size=1, alpha=alpha) +
    geom_line(position=dodge, alpha=alpha) +
...
p <- themefy(p)
# if(!out){plot(p)}else{png('expt2linPred.png', width=3.5, height=2.1, units='in', res=600); plot(p); dev.off()}
```
-->
...
### Full linear model
```{r}
m <- lmer(RT~targSpeed_ctr*maskSpeed_ctr+(1|SubjID),md)
pvalfn(m)
write.table(pvalfn(m),'clipboard',sep='\t',row.names=T)
...
m_vfast <- lmer(RT~targSpeed_ctr*maskSpeed_ctr+(1|SubjID),vfm)
pvalfn(m_vfast)
...
m <- lmer(RT~targSpeed_ctr*maskSpeed_ctr+(1|SubjID),md)
dsna <- ddply(md, .(SubjID, subjThresh_ctr, maskSpeed_ctr, targSpeed_ctr), summarise, 
              nNa=sum(is.na(RT)))
head(dsna)
